class HealthBar {
    constructor(scene, character, team, arbiter) {
        ...
        this.background = this.scene.add.rectangle(this.helper.getHorizontalPosition(character.position, this.team)-this.size/2, 
                                this.helper.floor+this.offSet, this.size, 10, 0x151515);
        
        // Create healthbar
        this.bar = this.scene.add.rectangle(this.helper.getHorizontalPosition(this.character.position, this.team)-this.size/2, 
    ...
    update() {
        this.destroy()
        this.background = this.scene.add.rectangle(this.helper.getHorizontalPosition(this.character.position, this.team)-this.size/2, 
                                this.helper.floor+this.offSet, this.size, 10, 0x151515);
        this.bar = this.scene.add.rectangle(this.helper.getHorizontalPosition(this.character.position, this.team)-this.size/2, 
                                this.helper.floor+this.offSet, this.size, 10, 0x992525);
